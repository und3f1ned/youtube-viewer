...
const writeTorConfig = async (startPort, count) => {
  ...
  await execWithPromise('echo "MaxCircuitDirtiness 10" >> /etc/tor/torrc');
  const promiseArr = [];
  for (let i = 0; i < count; i += 1) {
    const port = startPort + i;
    promiseArr.push(
      execWithPromise(`echo "SocksPort ${port}" >> /etc/tor/torrc`).then(() =>
        logger.debug(`PORT ${port} written in tor config`)
      )
    );
  }
...
const stopTor = async () => {
  ...
  try {
    ...
    logger.warn(
      'Failed to stop TOR. Usually this is a no op but ensure the subsequent attempts are using different IPs.'
    );
    logger.debug(error);
  }
};
